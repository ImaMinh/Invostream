...
async def batch_setup(chunk: list[UploadFile]) -> list[DuplicateFileInfo]:
    ...
    and enqueue only novel files. 
    Returns a list of DuplicateFileInfo for files that were skipped.
    """
    try:
        ...
        for file_name, file_bytes, content_hash in file_data:
            if content_hash in existing_hashes or content_hash in seen_in_batch:
                ...
                print(f"<--BATCH--> File already exists, skipping duplicate file: {file_name} (hash: {content_hash[:12]}...)")
            else:
    ...
    try:
        ...
        for extracted_data in extraction_results:
            try: 
                ...
                # --- CDC Pipeline: ClickHouse now reads natively from Kafka! ---
...
async def main_process():
    ...
    while True:
        ...
        future = loop.run_in_executor(executor, run_worker, file_paths, batch_id)
