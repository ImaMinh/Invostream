 ...
 * - Crisp solid outline border along outer 1.5px edge
 * - Interactive gradient blue border beam running ONCE (1.2s) on hover
 * - Subtle grid texture overlay on card background
 * - Theme-aware shadow intensity (rich shadow-lg on light mode, deep shadow-xl on dark mode)
 */
const BentoCard = ({ children, className = '', innerClassName = '', isScrolled = false, disableHover = false, ...props }) => {
  return (
    <div 
      className={`relative ${disableHover ? '' : 'group'} rounded-none p-[1px] transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] overflow-hidden ${
        isScrolled ? 'shadow-lg shadow-slate-300/80' : 'shadow-xl'
    ...
    >
      ...
      <div 
        className={`relative z-10 w-full h-full rounded-none transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] overflow-hidden bg-[var(--bento-inner-bg)] border border-solid ${innerClassName || 'border-[var(--bento-inner-border)]'} ${disableHover ? '' : 'group-hover:border-transparent'}`}
      >
